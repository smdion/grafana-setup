...
INFLUXDB_SERVER = "10.10.10.15" # IP or hostname to InfluxDB server
...
sudo chmod +x /etc/telegraf/pihole.py
sudo cat > /lib/systemd/system/piholemonitor.service << EOF
[Unit]

[Service]
ExecStart=/etc/telegraf/pihole.py
...
sudo systemctl enable piholemonitor.service
sudo systemctl start piholemonitor.service
